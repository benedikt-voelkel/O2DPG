...
module_name = "o2dpg_sim_metrics"
spec = importlib.util.spec_from_file_location(module_name, join(O2DPG_ROOT, "MC", "utils", "o2dpg_sim_metrics.py"))
o2dpg_sim_metrics = importlib.util.module_from_spec(spec)
sys.modules[module_name] = o2dpg_sim_metrics
spec.loader.exec_module(o2dpg_sim_metrics)
from o2dpg_sim_metrics import ResourcesDirect
...
class WorkflowExecutor:
    ...
    def __init__(self, workflowfile, args, jmax=100):
      ...
      self.resource_estimator = ResourcesDirect()
      self.resource_estimator.set_default_resources_from_workflow(self.workflowspec)
    ...
    def ok_to_submit(self, tid, backfill=False):
      ...
      newcpu = self.resource_estimator.get_resource_estimate(tid, ResourcesDirect.INDEX_CPU)
      newmem = self.resource_estimator.get_resource_estimate(tid, ResourcesDirect.INDEX_USS)
      if not backfill:
          # analyse CPU
          okcpu = (self.curcpubooked + newcpu <= self.cpulimit)
          # analyse MEM
          okmem = (self.curmembooked + newmem <= self.memlimit)
          ...
          if newcpu > 0.9*float(self.args.cpu_limit):
              return False
          if newmem/float(self.args.cpu_limit) >= 1900:
              return False

          # analyse CPU
          okcpu = (self.curcpubooked_backfill + newcpu <= self.cpulimit)
          okcpu = okcpu and (self.curcpubooked + self.curcpubooked_backfill + newcpu <= softcpufactor*self.cpulimit)
          # analyse MEM
          okmem = (self.curmembooked + self.curmembooked_backfill + newmem <= softmemfactor*self.memlimit)
    ...
    def book_resources(self, tid, backfill = False):
        ...
        # semaphores
        task = self.workflowspec['stages'][tid]
        sem = task.get('semaphore')
        if sem != None:
          self.semaphore_values[sem]+=1

        self.maxmemperid[tid] = self.resource_estimator.get_resource_estimate(tid, ResourcesDirect.INDEX_USS)
        self.cpuperid[tid] = self.resource_estimator.get_resource_estimate(tid, ResourcesDirect.INDEX_CPU)
        actionlogger.info("Book for %s, CPU: %d, MEM: %d", task["name"], self.cpuperid[tid], self.maxmemperid[tid])
    ...
    def monitor(self, process_list):
        ...
        for tid, proc in process_list:
            ...
            self.resource_estimator.add(tid, totalCPU, totalUSS/1024./1024., totalPSS/1024./1024., memory_extracted)
       ...
       for p in list(process_list):
          ...
          if returncode!=None:
            ...
            if returncode != 0:
               ...
               # from now on we consider the resources taken by this task as valid
               self.resource_estimator.mark_resources_as_valid(tid)
